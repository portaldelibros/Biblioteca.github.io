...
// Selección de todos los elementos con la clase mostrar-descripcion
const botonesDetalles = document.querySelectorAll('.mostrar-descripcion');

// Añadir un evento de clic a cada botón
botonesDetalles.forEach(boton => {
    boton.addEventListener('click', function(event) {
        event.preventDefault(); // Evitar el comportamiento predeterminado del enlace
        const index = parseInt(boton.getAttribute('data-index')); // Obtener el índice del atributo data-index
        mostrarDescripcion(index); // Llamar a la función mostrarDescripcion con el índice correcto
    });
});
